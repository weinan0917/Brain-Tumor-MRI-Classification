...
def default_config_path():
    return Path(__file__).resolve().with_name("config_vit_cifar100.yaml")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Fine-tune an ImageNet21k-pretrained ViT on CIFAR-100 resized to 224."
    )
    ...
    args.run_name = args.run_name or cfg_get(
        config, "train", "run_name", "vit_base_patch16_224_augreg_in21k_cifar100_224"
    )
    ...
    args.eval_test = bool(cfg_get(config, "train", "eval_test", False))
    ...
    args.pretrained = (
        bool(cfg_get(config, "model", "pretrained", True))
    ...
    args.resize_only = bool(cfg_get(config, "augment", "resize_only", True))
    ...
    args.resume = Path(args.resume) if args.resume else None
    return args
    ...
    def __init__(self, root, split, transform=None):
        ...
        if not path.exists():
            raise FileNotFoundError(
                f"Could not find CIFAR-100 split file: {path}. "
                "Expected files named train, val, and test."
            )
...
def build_datasets(args, model):
    ...
    if args.train_hflip > 0:
        train_steps.append(transforms.RandomHorizontalFlip(p=args.train_hflip))
    if args.color_jitter > 0:
        train_steps.append(transforms.ColorJitter(args.color_jitter))
    train_steps.extend(
        [
            transforms.ToTensor(),
    ...
    train_set = Cifar100PickleDataset(args.data_root, "train", transform=train_tfms)
    val_set = Cifar100PickleDataset(args.data_root, "val", transform=eval_tfms)
    test_set = Cifar100PickleDataset(args.data_root, "test", transform=eval_tfms)
